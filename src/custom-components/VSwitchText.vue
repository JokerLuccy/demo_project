<template>
  <div ref="vSwitchText" class="v-switch-text">
    <span
      v-for="(item, index) in getPropValue"
      :key="index"
      :class="{
        [item]: item.startsWith('icon') ? item : '',
        active: currentContext === item ? 'active' : '',
      }"
      class="iconfont"
      >{{ item.startsWith("icon") ? "" : item }}</span
    >
    <div
      ref="circle"
      :style="{ left: vSwitchWidth - circleWidth + 'px' }"
      class="circle"
    ></div>
  </div>
</template>
